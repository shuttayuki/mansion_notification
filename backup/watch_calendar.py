...
def load_subscribers():
    ...
    log_message(f"送信先数: {len(subscribers)}")
    return subscribers
...
def line_push(to_id, text):
    ...
    try:
        ...
        log_message(f"LINE通知送信成功: {to_id}")
        return True
    except requests.exceptions.RequestException as e:
        log_message(f"LINE通知送信失敗: {to_id} - {e}")
        return False
...
監視システムより自動通知"""
                    ...
                    log_message(f"通知完了: {success_count}/{len(subscribers)} 件成功")
            ...
            if attempt < 3:
                ...
                for subscriber in subscribers:
                    line_push(subscriber, error_message)
